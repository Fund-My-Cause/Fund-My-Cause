...
import { buildTrendForecast, compareCampaigns } from "@/lib/predictiveModeling";
...
export function PredictiveAnalytics({
  ...
  const averageProbability = Math.round(
    benchmarks.reduce((sum, item) => sum + item.successProbability, 0) /
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Real-time campaign forecasting and benchmarking.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400">
          <span
            className={`px-2 py-1 rounded-full ${connected ? "bg-emerald-900 text-emerald-300" : "bg-gray-800 text-gray-400"}`}
          >
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="bg-gray-950/60 border border-gray-800 p-4 rounded-2xl">
          ...
          <p className="text-lg font-semibold text-white mt-3 truncate">
            {leadCampaign?.title || "—"}
          </p>
          <p className="text-sm text-gray-400 mt-1">
            {leadCampaign?.successProbability}% probability
          </p>
        </div>
        <div className="bg-gray-950/60 border border-gray-800 p-4 rounded-2xl">
          <p className="text-xs text-gray-400 uppercase tracking-[0.2em]">
            Campaigns benchmarked
          </p>
          <p className="text-3xl font-semibold text-white mt-3">
            {benchmarks.length}
          </p>
